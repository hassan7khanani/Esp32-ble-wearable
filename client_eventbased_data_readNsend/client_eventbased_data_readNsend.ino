...
String responseValue4 = "";
String responseValue5 = "";

String newVal1 = "reboot\n";
String newVal2 = "fram,X,Y(,Z)\n";
String newVal3 = "fram,1,08\n";
//String newVal4 = "data_sync,"+ responseValue3 +"\n";
String newVal4 = "data_sync,1684284581\n";
String newVal5 = "battery\n";

char* responseValue_CONV;
...
static void notifyCallback(
  ...
  size_t length,
  bool isNotify) {
    Serial.print("Notify callback "); Serial.print(RED);
    Serial.print(pBLERemoteCharacteristic->getUUID().toString().c_str());
    Serial.print(RESET_C);  Serial.print(" of data length ");
    Serial.print(RED);  Serial.println(length);  Serial.print(RESET_C);
    Serial.println();  Serial.println();  Serial.print("data: ");
    Serial.print(BLUE);  Serial.write(pData, length);
    Serial.println();  Serial.print(RESET_C);

//BLE TX EVENT FROM PERIPHERAL

    if (length >= 2 && pData[0] == '1' && pData[1] == ',') {
      // Identifier is "1,"
      // Do not store the data in responseValue_CONV
    } else {
      if (responseValue_CONV != nullptr) {
        delete[] responseValue_CONV;  // Deallocate previously allocated memory
      }

      responseValue_CONV = new char[length + 1];  // Allocate memory based on data length (+1 for null terminator)
      memcpy(responseValue_CONV, reinterpret_cast<char*>(pData), length);  // Copy data to responseValue_CONV

      responseValue_CONV[length] = '\0';  // Null terminate the string

      Serial.print("response_conv value is: ");
      Serial.println(responseValue_CONV);
    }

    if(sent1 == true){

        responseValue1 = responseValue_CONV;
        Serial.println("REBOOT: ");
        Serial.println(responseValue1);
        sent1 = false;
    }
    if(sent2 == true){

        responseValue2 = responseValue_CONV;
        Serial.println("FRAM_XYZ: ");
        Serial.println(responseValue2);
        sent2 = false;
    }

    if(sent3 == true){
        
        responseValue3 = String(responseValue_CONV); 
        
        Serial.println("FRAM UNIX: ");
        Serial.println(responseValue3);
        sent3 = false;
    }

    if(sent4 == true){

        responseValue4 = responseValue_CONV; // data_sync string receiving
        Serial.println("DATA_SYNC: ");
        Serial.println(responseValue4);
        sent4 = false;
    }
    ...
    if(sent5 == true){
        ...
        responseValue5 = responseValue_CONV; //battery value receiving
        Serial.println("BATTERY: ");
        Serial.println(responseValue5);

        if(responseValue5 != "1,A,B,C,D,E,F,G,H"){  
            int responseValue5Int = responseValue5.toInt();
            float voltage = (responseValue5Int/4095.0)*7.2;
...
void reboot_fn(){
  sent1 = true;
  Serial.println(GREEN);
  Serial.println("=== reboot_fn ===");
  pRemoteCharacteristicRX->writeValue(newVal1.c_str(), newVal1.length());
  Serial.println(RESET_C);
}

void framXYZ_fn(){
  sent2 = true;
  Serial.println(GREEN);
  Serial.println("=== framXYZ_fn ===");
...
void loop() {
  ...
  if (connected) {
    ...
    reboot_fn();
    Serial.println("FUNCTION 1 EXIT");
    delay(2000);
    framXYZ_fn();
    Serial.println("FUNCTION 2 EXIT");
    delay(2000);
    framUNIX_fn();
    Serial.println("FUNCTION 3 EXIT");
    delay(2000);
    datasync_fn();
    Serial.println("FUNCTION 4 EXIT");
    delay(2000);
    battery_fn();
    Serial.println("FUNCTION 5 EXIT");
    delay(2000);
